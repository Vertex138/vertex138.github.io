(() => {
  ...
  function getViewedImageCount() {
    try {
      ...
      return new Set(storedIds.map(Number).filter(imageId => Number.isInteger(imageId) && imageId >= 1 && imageId <= TOTAL_IMAGE_GOAL)).size;
    } catch (error) {
      return 0;
    }
  }
  ...
  function validateFaqData(faqData) {
    if (faqData === null || typeof faqData !== "object" || Array.isArray(faqData)) {
      throw new Error("faq.json must map questions to answers.");
    }
    const entries = Object.entries(faqData).filter(([question, answer]) => question.trim() !== "" && typeof answer === "string" && answer.trim() !== "");
    if (entries.length === 0) {
      throw new Error("faq.json does not contain any valid entries.");
    }
    return entries;
  }
  ...
  async function loadFaq() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Could not load faq.json: ${response.status}`);
      }
      const entries = validateFaqData(await response.json());
      const fragment = document.createDocumentFragment();
      entries.forEach(([question, answer]) => {
        fragment.append(createFaqEntry(question, answer));
      });
      ...
      status.textContent = "Jeff's FAQ could not be loaded.";
