(() => {
  ...
  function menuLinkMarkup({ label, href, required, hidden }) {
    const initialLabel = hidden && required > 0 ? "? ? ? ? ? ? ?" : label;
  ...
  const unlockNotice = document.getElementById("site-menu-unlock-notice");
  ...
  function getStoredUnlockLevel(key) {
    try {
      const savedLevel = Number(localStorage.getItem(key));

      return UNLOCK_LEVELS.includes(savedLevel) ? savedLevel : 0;
    } catch (error) {
      console.warn("Could not retrieve the menu unlock state:", error);
  ...
  function saveStoredUnlockLevel(key, level) {
    try {
      ...
      console.warn("Could not save the menu unlock state:", error);
    }
  }
  ...
  function clearPendingUnlockNotice() {
    try {
      ...
      console.warn("Could not clear the menu unlock notice state:", error);
    }
  }
  ...
  function startUnlockAnimation() {
    ...
    if (unlockNotice.hidden || root.classList.contains("reduced-motion")) {
      return;
    }
  ...
  function announceNewUnlock(viewedCount) {
    ...
    const knownLevel = getStoredUnlockLevel(STORAGE_KEYS.unlockKnown);

    let reachedNewLevel = false;

    if (highestUnlockedLevel > knownLevel) {
      saveStoredUnlockLevel(STORAGE_KEYS.unlockKnown, highestUnlockedLevel);

      saveStoredUnlockLevel(STORAGE_KEYS.unlockPending, highestUnlockedLevel);

      reachedNewLevel = true;
    }

    const pendingLevel = getStoredUnlockLevel(STORAGE_KEYS.unlockPending);
  ...
  function refreshProgress() {
    ...
    document.querySelectorAll("[data-unlock]").forEach((link) => {
      ...
      const hideLabelWhenLocked = link.dataset.hiddenLabel === "true";

      const labelElement = link.querySelector(".site-menu-item-label");

      const status = link.querySelector(".site-menu-unlock-status");
      ...
      if (unlocked) {
        ...
        status.textContent = `LOCKED • ${viewedCount} / ${required}`;
      }
    });

    const collectionComplete = viewedCount >= TOTAL_IMAGE_GOAL;

    forceNewButton.disabled = collectionComplete || !forceNewReady;
  ...
  accessibilityButton.addEventListener("click", openAccessibilityOverlay);

  accessibilityCloseButton.addEventListener("click", closeAccessibilityOverlay);
